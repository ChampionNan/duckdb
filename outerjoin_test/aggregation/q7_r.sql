...
create or replace temp view aggt as select t2.src, min(annot) as annot from t2 right outer join aggk on t2.dst = aggk.src group by t2.src;
create or replace temp view aggs as select s2.src, min(annot) as annot from s2 right outer join aggt on s2.dst = aggt.src group by s2.src;
select aggs.src, min(annot) as total from r1 right outer join aggs on r1.dst = aggs.src group by aggs.src;